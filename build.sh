...
docker run -ti -w $(pwd) -v $(pwd):$(pwd) -v /tmp/maven:/tmp/maven -e ENABLE_JAR=1 --rm registry.lazycat.cloud/lzc/lzc-api-protoc env PROTOC_GEN_GO=1 PROTOC_GEN_JS=1 PROTOC_GEN_JAVA=0 ./tools/gen.sh
